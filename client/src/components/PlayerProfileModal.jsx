...
export default function PlayerProfileModal({ pseudoId, playerId, onClose }) {
  ...
  const { players, allBets, results, scoreboard, player: me, myBets, challenges, matches, logout } = useApp()
  const [challengeMatchId, setChallengeMatchId] = useState(null)
  const [tab, setTab] = useState('paris')

  const player = players[id]
  const playerBets = allBets[id] || {}

  const board = useMemo(() => scoreboard(), [scoreboard])
  const rank = board.findIndex((p) => p.pseudoId === id) + 1
  const playerData = board.find((p) => p.pseudoId === id)
  ...
  }, [me, id, myBets, playerBets, results, matches])
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-card player-profile-modal" onClick={(e) => e.stopPropagation()}>
        ...
        {tab === 'oppositions' && oppositeMatches.length > 0 && (
          <div className="ppm-bets">
            {oppositeMatches.map((m) => {
              ...
              const id1 = `${m.id}_${me.pseudoId}_vs_${id}`
              const id2 = `${m.id}_${id}_vs_${me.pseudoId}`
        ...
        {(tab === 'paris' || oppositeMatches.length === 0) && (
          <div className="ppm-bets">
            {phases.length === 0 ? (
